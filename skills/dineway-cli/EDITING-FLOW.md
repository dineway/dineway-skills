...
Dineway stores rich text as [Portable Text](https://portabletext.org/) (PT) — a structured JSON format. The CLI automatically converts between PT and markdown so you work with a familiar text format.
...
```bash
npx dineway content get posts 01ABC123 --raw
```
...
```
DinewayApiError: Content has been modified since last read (version conflict)
  status: 409
  code: CONFLICT
```
