...
`siteUrl` replaces `passkeyPublicOrigin` (which only fixed passkeys). It applies to passkeys, CSRF origin matching, OAuth redirects, login redirects, MCP discovery, snapshot exports, sitemap, robots.txt, schemamap, and JSON-LD structured data. Production builds must set `siteUrl` or `DINEWAY_SITE_URL` whenever the request origin is not the public browser-facing origin.
...
When deploying a Dineway project to Forgeway:
